    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="space-y-3">
          <Button variant="outline" size="sm" className="w-full h-8 text-xs justify-start" onClick={async () => { let id:any; try { const { useEditorContext } = require('@/contexts/EditorContext'); const { downloadAsFile } = require('@/hooks/use-file-system'); const { markdownToHtml } = require('@/lib/markdown'); const { editorState, saveContent } = useEditorContext(); id = startTask('导出 PDF', 5); await saveContent(); updateTask(id, { progress: 30 }); const html = `<!doctype html><html><head><meta charset="utf-8"><title>导出</title></head><body>${markdownToHtml(editorState.content)}</body></html>`; await downloadAsFile('untitled-1.pdf', html, 'application/pdf'); completeTask(id); } catch(e:any) { try { failTask?.(id, e?.message || '导出失败'); } catch {} console.error(e); } }} >
            <FileText className="w-4 h-4 mr-2" />
            导出为 PDF
          </Button>
          
          <Button variant="outline" size="sm" className="w-full h-8 text-xs justify-start" onClick={async () => { let id:any; try { const { useEditorContext } = require('@/contexts/EditorContext'); const { downloadAsFile } = require('@/hooks/use-file-system'); const { editorState, saveContent } = useEditorContext(); id = startTask('导出 DOCX', 5); await saveContent(); updateTask(id, { progress: 30 }); await downloadAsFile('untitled-1.docx', editorState.content, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'); completeTask(id); } catch(e:any) { try { failTask?.(id, e?.message || '导出失败'); } catch {} console.error(e); } }} >
